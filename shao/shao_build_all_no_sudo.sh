...
print_usage() {
  ...
  echo
  echo "Options:"
  echo " -s <system>     Target system, supported values are osx (default), centos, x86"
...
while getopts "A:ah?s:cm:j:p:w:WPoiC:cR:rY:yO:ULx:eE:" opt; do
  case "$opt" in
  [h?])
    print_usage
    exit 0
...
if [ $load_system_dependencies == yes ]; then
  load_s_dependencies
fi
